...
export const BrowserValidatorRuntime = ({
  validatorId,
  hubUrl = "ws://localhost:8081",
}: {
  validatorId: string;
  hubUrl?: string;
}) => {
  ...
  useEffect(() => {
    ...
    async function openConnection() {
      ...
      if (!walletSignMessage) {
        setErrors(
          "Wallet does not support message signing. Enable message signing in your wallet settings.",
        );
        return;
      }
      ...
      ws.onopen = async () => {
        ...
        try {
          ...
          const signatureBytes = await walletSignMessage(
        ...
        try {
          ...
          if (incoming.type === "signup") {
            ...
            const payload: ValidateMessagePayload =
              incoming.data as ValidateMessagePayload;
            ...
            try {
              ...
              const sigBytes = await walletSignMessage(
                new TextEncoder().encode(replyMessage),
              );
              ws.send(
                JSON.stringify({
                  type: "validate",
                  data: {
                    ...
                    signedMessage: JSON.stringify(Array.from(sigBytes)),
                  },
                }),
              );
            } catch (e) {
  ...
  if (errors)
    return (
      <div className="text-red-500 text-xs">
        Validator runtime error: {errors}
      </div>
    );
  const stateLabel = errors
    ? "error"
    : active
      ? signupDone
        ? "active"
  ...
  return (
    <div className="text-xs text-muted-foreground">
      Validator runtime: {stateLabel} (attempt {attemptRef.current + 1}).
    </div>
  );
};
