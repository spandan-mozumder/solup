...
async function resolveClientInfo(req: NextRequest) {
  const forwarded =
    req.headers.get("x-forwarded-for") || req.headers.get("forwarded");
  let ip = "unknown";
  if (forwarded) {
  ...
  try {
    const geoResp = await fetch(
      `https://ipapi.co/${ip === "127.0.0.1" ? "" : ip}/json/`,
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      "Registering validator for user:",
      userId,
      "publicKey:",
      publicKey,
    );
    const { ip: detectedIp, location: detectedLocation } =
      await resolveClientInfo(req);
    ...
    try {
      ...
      const byUser = await prismaClient.validator.findFirst({
        where: { userId },
      });
      if (byUser) {
        if (byUser.publicKey !== publicKey) {
          const pkInUse = await prismaClient.validator.findFirst({
            where: { publicKey },
          });
          if (pkInUse && pkInUse.id !== byUser.id) {
            return new Response(
              JSON.stringify({ error: "Public key already registered" }),
              { status: 409 },
            );
          }
          const updated = await prismaClient.validator.update({
            ...
            data: {
              publicKey,
              ip: detectedIp || byUser.ip,
              location: detectedLocation || byUser.location,
            },
          });
      ...
      const pkExisting = await prismaClient.validator.findFirst({
        where: { publicKey },
      });
      ...
      if (pkExisting && !pkExisting.userId) {
        const claimed = await prismaClient.validator.update({
          ...
          data: {
            userId,
            ip: detectedIp || pkExisting.ip,
            location: detectedLocation || pkExisting.location,
          },
        });
